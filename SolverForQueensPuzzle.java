...
public class SolverForQueensPuzzle {
    ...
    private void recordSolutionsStarted() {
        ...
        // Which has been requested, a base case or recursive case?
            if (inProgress.lastRankFilled == 
			inProgress.filesWithQueens.length - 1){
			// action(s) for base case(s)
				if (!inProgress.lastIsNg())
					solutions.add(inProgress);
				nBoardsConsidered++;
						 //if not solution is done
				...
				inProgress.populate(0);
				// action for recursive cases
				for (int i = 0; i < inProgress.filesWithQueens.length; i++){
					...
					if (inProgress.filesWithQueens[inProgress.lastRankFilled] ==
						inProgress.filesWithQueens.length - 1)
						inProgress.depopulate();
				}
		}
    ...
    public SolverForQueensPuzzle( int size) {
        solutions = new ArrayList<BoardForQueensPuzzle>();
